...
class Products_model extends CI_Model
{
    ...
    var $order = array('te_products.pid' => 'desc'); // default order
    ...
    {
        $this->db->select('te_products.*,te_product_cat.title AS c_title,te_warehouse.title');
        $this->db->from($this->table);
        $this->db->join('te_warehouse', 'te_warehouse.id = te_products.warehouse');
        if ($sub) {
            $this->db->join('te_product_cat', 'te_product_cat.id = te_products.sub_id');
            ...
            $this->db->where("te_products.sub_id=$id");

        } else {
            $this->db->join('te_product_cat', 'te_product_cat.id = te_products.pcat');

            if ($w) {

                if ($id > 0) {
                    $this->db->where("te_warehouse.id = $id");
                    // $this->db->where('te_products.sub_id', 0);
                }
                if ($this->aauth->get_user()->loc) {
                    $this->db->group_start();
                    $this->db->where('te_products.loc', $this->aauth->get_user()->loc);
                ...
                if ($id > 0) {
                    $this->db->where("te_product_cat.id = $id");
                    $this->db->where('te_products.sub_id', 0);
                }
            }
        }
    ...
    public function addnew($catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $v_type, $v_stock, $v_alert, $wdate, $code_type, $w_type = '', $w_stock = '', $w_alert = '', $sub_cat = '', $b_id = '', $serial = '')
    {
        ...
        if ($this->aauth->get_user()->loc) {
            if ($ware_valid['loc'] == $this->aauth->get_user()->loc OR $ware_valid['loc'] == '0' OR $warehouse == 0) {
                ...
                $this->db->trans_start();
                if ($this->db->insert('te_products', $data)) {
                ...
                if ($serial) {
                    ...
                    $this->db->insert_batch('te_product_serials', $serial_group);
                }
                if ($v_type) {
                    foreach ($v_type as $key => $value) {
                        if ($v_type[$key] && numberClean($v_stock[$key]) > 0.00) {
                            ...
                            $this->db->insert('te_products', $data);
    ...
    public function edit($pid, $catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $code_type, $sub_cat = '', $b_id = '', $vari = null, $serial = null)
    {
        ...
        if (isset($serial['old'])) {
            $this->db->delete('te_product_serials', array('product_id' => $pid,'status'=>0));
            $serial_group = array();
...
COUNT(IF( te_products.qty > 0, te_products.qty, NULL)) AS instock,
COUNT(IF( te_products.qty <= 0, te_products.qty, NULL)) AS outofstock,
COUNT(te_products.qty) AS total
FROM te_products $whr");
        echo json_encode($query->result_array());
    }

    public function products_list($id, $term = '')
    {
        $this->db->select('te_products.*');
        $this->db->from('te_products');
        $this->db->where('te_products.warehouse', $id);
        $this->db->where('te_products.loc', $this->aauth->get_user()->loc);
        if ($this->aauth->get_user()->loc) {
            $this->db->join('te_warehouse', 'te_warehouse.id = te_products.warehouse');
            $this->db->where('te_warehouse.loc', $this->aauth->get_user()->loc);
        ...
        if ($term) {
            $this->db->where("te_products.product_name LIKE '%$term%'");
            $this->db->or_where("te_products.product_code LIKE '$term%'");
        }
        $query = $this->db->get();
    ...
    public function transfer($from_warehouse, $products_l, $to_warehouse, $qty)
    {
        ...
        foreach ($products_l as $row) {
            ...
            if ($c_qty - $qty < 0) {
                ...
                if ($c_pid) {
                    ...
                    $this->db->delete('te_movers', array('d_type' => 1, 'rid1' => $row));
        ...
        if ($move) {
            $this->db->update_batch('te_products', $updateArray, 'pid');
        }
